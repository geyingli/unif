...
class ALBERTLM(BERTLM, LMModule):
    ...
    def _set_placeholders(self, target, on_export=False, **kwargs):
        self.placeholders = {
            ...
            "masked_lm_positions": common.get_placeholder(target, "masked_lm_positions", [None, self._max_predictions_per_seq * (1 + self._do_permutation)], tf.int32),
            "masked_lm_ids": common.get_placeholder(target, "masked_lm_ids", [None, self._max_predictions_per_seq * (1 + self._do_permutation)], tf.int32),
            "masked_lm_weights": common.get_placeholder(target, "masked_lm_weights", [None, self._max_predictions_per_seq * (1 + self._do_permutation)], tf.float32),
            "sentence_order_labels": common.get_placeholder(target, "sentence_order_labels", [None], tf.int32),
        }
        if not on_export:
...
def get_albert_config(config_file=None):
    if not os.path.exists(config_file):
        raise ValueError(
            "Can\"t find config_file \"%s\". Please pass the correct path of configuration file, "
