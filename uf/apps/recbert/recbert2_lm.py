...
from .recbert2 import RecBERT2, sample_wrong_tokens, get_decay_power
...
class RecBERT2LM(LMModule):
    """ Language modeling on RecBERT-2. """
    ...
    def convert(self, X=None, y=None, sample_weight=None, X_tokenized=None, is_training=False, is_parallel=False):
        ...
        if X is not None or X_tokenized is not None:
            ...
            (input_tokens, input_ids,
             add_label_ids, rep_label_ids, del_label_ids, cls_label_ids) = self._convert_X(X_target, tokenized=tokenized, is_training=is_training)
            data["input_ids"] = np.array(input_ids, dtype=np.int32)

            if is_training:
                data["add_label_ids"] = np.array(add_label_ids, dtype=np.int32)
                data["rep_label_ids"] = np.array(rep_label_ids, dtype=np.int32)
    ...
    def _convert_X(self, X_target, tokenized, is_training):
        ...
        for idx, sample in enumerate(X_target):
            ...
            if is_training:
                if len(_input_tokens) == 0 or len(_input_tokens) > self.max_seq_length - 2:
                    continue
            else:
        ...
        for idx in range(len(tokenized_input_ids)):
            ...
            if is_training:
                ...
                max_rep = np.sum(np.random.random(nonpad_seq_length) < self._rep_prob)
                max_del = np.sum(np.random.random(nonpad_seq_length) < self._del_prob)

                ok = sample_wrong_tokens(
                    _input_ids, _add_label_ids, _rep_label_ids, _del_label_ids,
                    max_add=max_add, max_rep=max_rep, max_del=max_del,
            ...
            _rep_label_ids += [0] * (self.max_seq_length - len(_rep_label_ids))
        ...
        return input_tokens, input_ids, add_label_ids, rep_label_ids, del_label_ids, cls_label_ids
    ...
    def _set_placeholders(self, **kwargs):
        self.placeholders = {
            ...
            "rep_label_ids": tf.placeholder(tf.int32, [None, self.max_seq_length], "rep_label_ids"),
    ...
    def _forward(self, is_training, placeholders, **kwargs):
        ...
        model = RecBERT2(
            ...
            rep_label_ids=placeholders["rep_label_ids"],
    ...
    def _get_fit_info(self, output_arrays, feed_dict, from_tfrecords=False):
        ...
        if from_tfrecords:
            batch_inputs = output_arrays[-5]
            batch_add_labels = output_arrays[-4]
            ...
            batch_rep_labels = feed_dict[self.placeholders["rep_label_ids"]]
        ...
        rep_accuracy = np.sum((batch_rep_preds == batch_rep_labels) * batch_mask) / (np.sum(batch_mask) + 1e-6)
        ...
        # cls accuracy
        batch_cls_preds = output_arrays[3]
        cls_accuracy = np.mean((batch_cls_preds == batch_cls_labels))

        # add loss
        batch_add_losses = output_arrays[4]
        add_loss = np.mean(batch_add_losses)

        # rep loss
        batch_rep_losses = output_arrays[5]
        ...
        # cls loss
        batch_cls_losses = output_arrays[7]
    ...
    def _get_predict_outputs(self, output_arrays, n_inputs):
        ...
        rep_preds = com.transform(output_arrays[1], n_inputs)
        del_preds = com.transform(output_arrays[2], n_inputs)
                ...
                for i in range(_input_length):
                    if i > 0:
                        ...
                        if self._rep_prob > 0 and _rep_preds[i] != 0:
                            _token = "{rep:%s}" % self.tokenizer.convert_ids_to_tokens([_rep_preds[i]])[0]
                ...
                for i in range(_input_length):
                    if i > 0:
                        ...
                        if self._rep_prob > 0 and _rep_preds[i] != 0:
                            _start_ptr = _mapping_start[i] + n
                            _end_ptr = _mapping_end[i] + n
                            _rep_token = _text[_start_ptr: _end_ptr]
                            _token = "{rep:%s}" % self.tokenizer.convert_ids_to_tokens([_rep_token])[0]
                            _text = _text[:_start_ptr] + _token + _text[_end_ptr:]
                            n += len(_token) - len(_rep_token)
                    if self._add_prob > 0 and _add_preds[i] != 0:
        ...
        # cls
        cls_probs = com.transform(output_arrays[3], n_inputs)
