""" AdaBERT, a distillation model of BERT.
    Code revised from Alibaba PAI Team's implementation. Link lost.
"""
...
class AdaBERTClsDistillor(BaseDecoder):
    def __init__(self,
                 bert_config,
                 is_training,
                 input_ids,
        ...
        student_inputs = [input_ids, input_mask, segment_ids, label_ids]
        if is_training:
            teacher = BERTEncoder(
                ...
                **kwargs)
            teacher_logits = []
            for i, sequence in enumerate(teacher.all_encoder_layers):
                teacher_logit = _get_logits(
                    sequence[:,0,:], bert_config.hidden_size, f"{scope}/layer_{i}/hidden", trainable=True,
                )
                teacher_logit = tf.expand_dims(teacher_logit, axis=1)
                teacher_logits.append(teacher_logit)
            teacher_logits = tf.concat(teacher_logits, axis=1)
            student_inputs = [input_ids, input_mask, segment_ids, teacher_logits, label_ids]

        student = AdaBERTStudent(
            ...
            seq_len=util.get_shape_list(input_ids)[1],
            keep_prob=bert_config.hidden_dropout_prob,
            temp_decay_steps=temp_decay_steps,
            # model_opt_lr=model_opt_lr,
            # arch_opt_lr=arch_opt_lr,
            model_l2_reg=model_l2_reg,
            arch_l2_reg=arch_l2_reg,
        ...
        if is_training:
            ...
            self._tensors["losses"] = tf.reshape(self.train_loss, [1])
        else:
...
class AdaBERTStudent(object):
    ...
    def _build_graph(self, inputs, is_training, global_step):
        ...
        dy_batch_size = tf.shape(word_ids)[0]
        ...
        with tf.device("cpu"):
            if self.pretrained_word_embeddings is not None:
                ...
                wemb = tf.get_variable(
                    ...
                    dtype=tf.float32,
                    initializer=util.xavier_initializer())
                pemb = tf.get_variable(
                    name="pemb",
        ...
        if self.given_arch is None:
            ...
            teacher_prob_logits = prob_logits
            # (13, bs, #classes)
        ...
        self.predictions = tf.argmax(self.logits, -1, name="preds")
