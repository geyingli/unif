...
class BERTTmpBinaryClassifier(BinaryClassifierModule):
    ...
    def convert(self, X=None, y=None, sample_weight=None, X_tokenized=None, is_training=False, is_parallel=False):
        ...
        if X is not None or X_tokenized is not None:
            ...
            input_ids, input_mask, segment_ids, label_weights = self._convert_X(X_tokenized if tokenized else X, tokenized=tokenized)
            data["input_ids"] = np.array(input_ids, dtype=np.int32)
            data["input_mask"] = np.array(input_mask, dtype=np.int32)
            data["segment_ids"] = np.array(segment_ids, dtype=np.int32)
            data["label_weights"] = np.array(label_weights, dtype=np.int32)
    ...
    def _convert_X(self, X_target, tokenized):
        ...
        for idx, sample in enumerate(X_target):
            ...
            try:
                segments = self._convert_x(sample["t"], tokenized)
            except Exception as e:
                raise ValueError("Wrong input format (%s): %s." % (sample, e))

            if "w" in sample:
                _label_weights = sample["w"]
        ...
        return input_ids, input_mask, segment_ids, label_weights
    ...
    def _set_placeholders(self, **kwargs):
        self.placeholders = {
            ...
            "label_weights": tf.placeholder(tf.float32, [None, self.label_size], "label_weights"),
    ...
    def _forward(self, is_training, placeholders, **kwargs):
        ...
        decoder = BinaryClsDecoder(
            ...
            label_weights=placeholders.get("label_weights"),
