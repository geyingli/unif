...
from ..modeling.base import CLSDecoder, BinaryCLSDecoder, SeqCLSDecoder, SeqCLSMultiTaskDecoder, MRCDecoder
...
class BERTSeqMultiTaskClassifier(BERTClassifier, ClassifierModule):
    ...
    def _convert_y(self, y):
        ...
        cls_label_ids = []
        for sample in y:
            sample = [label for label in sample["seq_cls"]]
            num_labels = len(sample)
            if num_labels < self.max_seq_length:
                sample.extend([0] * (self.max_seq_length - num_labels))
            elif num_labels > self.max_seq_length:
            ...
            _label_ids = [self._seq_cls_label_to_id[label] for label in sample]
    ...
    def _get_predict_outputs(self, batch_outputs):
        ...
        # probs
        seq_cls_probs = common.transform(output_arrays[0], n_inputs)
        cls_probs = common.transform(output_arrays[1], n_inputs)
    ...
    def _get_score_outputs(self, batch_outputs):
        ...
        # accuracy
        seq_cls_preds = common.transform(output_arrays[0], n_inputs)
        cls_preds = common.transform(output_arrays[1], n_inputs)
        ...
        # loss
        seq_cls_losses = common.transform(output_arrays[2], n_inputs)
        cls_losses = common.transform(output_arrays[3], n_inputs)
